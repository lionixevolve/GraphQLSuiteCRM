...
class QuotesListType extends AbstractListType
{
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
        ...
        $quotesBean = BeanFactory::getBean('AOS_Quotes');
        $searchParams = '';
        if($info!=null){
        ...
        if(!empty($args['limit'])){
            if($args['limit']-$offset> $sugar_config['list_max_entries_per_page']){
                ...
                for($i=0;$i<$loopTimes;$i++){
                    $fullList = $quotesBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
                ...
                $list = $quotesBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
            }
        }else{
            $list = $quotesBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
        }
