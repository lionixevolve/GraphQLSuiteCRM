...
class NotesListType extends AbstractListType
{
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
            ...
            if (in_array($key, $arrayKeys)) {
                if (substr_count($args[$key], ",")>0) {
                    ...
                    if(is_array($args[$key])){
                        ...
                        // This is because we are sending a parameter ids to this OpportunityListType
                        // Check AccountType for an example
                        $key='id';
                    }else{
        ...
        // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/lx.log', PHP_EOL .PHP_EOL.__FILE__ .":". __LINE__." -- ". print_r($searchParams, 1), FILE_APPEND);
        $offset=0;
        ...
        if(!empty($args['limit'])){
            if($args['limit']-$offset> $sugar_config['list_max_entries_per_page']){
                ...
                for($i=0;$i<$loopTimes;$i++){
                    $fullList = $noteBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
                ...
                $list = $noteBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
            }
        }else{
            $list = $noteBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
        }
