        ...
        if(!empty($args['limit'])){
            if($args['limit']-$offset> $sugar_config['list_max_entries_per_page']){
                ...
                for($i=0;$i<$loopTimes;$i++){
                    $fullList = $opportunityBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
                ...
                $list = $opportunityBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
            }
        }else{
            $list = $opportunityBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
        }
