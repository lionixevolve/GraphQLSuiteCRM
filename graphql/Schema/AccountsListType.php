...
class AccountsListType extends AbstractListType
{
    ...
    public function resolve($value = null, $args = [], ResolveInfo $info = null)
    {
        ...
        // file_put_contents($_SERVER['DOCUMENT_ROOT'].'lx.log', PHP_EOL .PHP_EOL.__FILE__ .":". __LINE__." -- ". print_r($key, 1), FILE_APPEND);
        ...
        if(!empty($args['limit'])){
            if($args['limit']-$offset> $sugar_config['list_max_entries_per_page']){
                ...
                for($i=0;$i<$loopTimes;$i++){
                    $fullList = $accountBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
                ...
                $list = $accountBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
            }
        }else{
            $list = $accountBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
        }
