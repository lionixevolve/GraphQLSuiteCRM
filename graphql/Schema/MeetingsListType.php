...
class MeetingsListType extends AbstractListType
{
    ...
    public function resolve($value = null, $args = [], $info = null)
    {
        ...
        // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/mylog.log', PHP_EOL .PHP_EOL.__FILE__ .":". __LINE__." -- ". print_r($searchParams, 1), FILE_APPEND);
        $offset=0;
        ...
        if(!empty($args['limit'])){
            if($args['limit']-$offset> $sugar_config['list_max_entries_per_page']){
                ...
                for($i=0;$i<$loopTimes;$i++){
                    $fullList = $meetingBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
                ...
                $list = $meetingBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
            }
        }else{
            $list = $meetingBean->get_list($orderBy, implode(" AND ", $searchParams), $offset);
        }
        ...
        if (is_array($list['list']) && !empty($list['list'])) {
            if($list['list'][0]->ACLAccess('list')){
                foreach ($list['list'] as $item) {
                    // file_put_contents($_SERVER['DOCUMENT_ROOT'].'/lx.log', PHP_EOL .PHP_EOL.__FILE__ .":". __LINE__." -- ". print_r($item->id, 1), FILE_APPEND);
